...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{list_of_cols <-}\StringTok{ }\KeywordTok{list}\NormalTok{(}
  \StringTok{"hot_colors"}\NormalTok{ =}\StringTok{ }\KeywordTok{get_icae_colors}\NormalTok{(}\DataTypeTok{n =} \DecValTok{3}\NormalTok{, }\DataTypeTok{palette_used =} \StringTok{"hot"}\NormalTok{),}
  \StringTok{"cold_colors"}\NormalTok{ =}\StringTok{ }\KeywordTok{get_icae_colors}\NormalTok{(}\DataTypeTok{n =} \DecValTok{2}\NormalTok{, }\DataTypeTok{palette_used =} \StringTok{"cool"}\NormalTok{, }
                                  \DataTypeTok{reverse_pal =} \OtherTok{TRUE}\NormalTok{),}
  \StringTok{"mixed_colors"}\NormalTok{ =}\StringTok{ }\KeywordTok{get_icae_colors}\NormalTok{(}\DecValTok{4}\NormalTok{, }\DataTypeTok{palette_used =} \StringTok{"mixed"}\NormalTok{)}
\NormalTok{)}
\KeywordTok{print}\NormalTok{(list_of_cols)}
\CommentTok{#> $hot_colors}
\CommentTok{#> [1] "#D8C469" "#AC6234" "#800000"}
\CommentTok{#> }
\CommentTok{#> $cold_colors}
\CommentTok{#> [1] "#002B80" "#8600B3"}
\CommentTok{#> }
\CommentTok{#> $mixed_colors}
\CommentTok{#> [1] "#006600" "#554F55" "#A1419A" "#800000"}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{get_icae_colors}\NormalTok{(}\KeywordTok{c}\NormalTok{(}\StringTok{"sand"}\NormalTok{, }\StringTok{"dark green"}\NormalTok{))}
\CommentTok{#>       sand dark green }
\CommentTok{#>  "#d8c469"  "#006600"}
\end{Highlighting}
\end{Shaded}

If you want to get the hex codes for all base colors of the corporate
design call:

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{get_icae_colors}\NormalTok{(}\StringTok{"all"}\NormalTok{)}
\CommentTok{#>     orange     purple dark green       sand  dark blue   dark red }
\CommentTok{#>  "#ff9900"  "#8600b3"  "#006600"  "#d8c469"  "#002b80"  "#800000"}
\end{Highlighting}
\end{Shaded}

\subsubsection{Update ggplot2 objects}\label{update-ggplot2-objects}

The key functions are \texttt{scale\_color\_icae()} and
\texttt{scale\_fill\_icae()}, which allow to control the \texttt{color}
and \texttt{fill} aesthetics in ggplot2 objects respectively.
...
\includegraphics{icae-vignette_files/figure-latex/unnamed-chunk-8-1.pdf}
...
\includegraphics{icae-vignette_files/figure-latex/unnamed-chunk-9-1.pdf}
...
\includegraphics{icae-vignette_files/figure-latex/unnamed-chunk-10-1.pdf}
